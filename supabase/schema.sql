-- ============================================================
-- Phil-Apex: COMPLETE SCHEMA (single file)
-- Consolidated from all migrations — safe to run on a fresh
-- Supabase project.  Every statement is idempotent.
-- ============================================================
...
-- ── applicant reference-number sequence ───────────────────────
...
create table if not exists public.applicants (
  ...
  peos_modules_completed integer not null default 0
    check (peos_modules_completed >= 0 and peos_modules_completed <= 8),
...
-- ── job_orders (Phase 1 + Phase 2 columns merged) ─────────────
create table if not exists public.job_orders (
  ...
  accreditation_id uuid references public.accreditations(id) on delete set null,
  foreign_partner_id uuid references public.foreign_partners(id) on delete set null,
  position text,
  class text check (class is null or class in ('direct', 'additional')),
  manpower_requested integer check (manpower_requested is null or manpower_requested >= 0),
  jo_validity_date date,
...
-- ── audit_log (applicant_id nullable for non-applicant events) ─
...
-- ============================================================
-- SEED: Document Requirements (reference data)
-- ============================================================
...
-- ── auto-close job orders when slots are filled ───────────────
...
-- ── public registration RPC (returns reference number) ────────
...
create trigger job_orders_sync_status
  ...
  for each row execute function public.sync_job_order_status();
...
-- Revoke default public access, then grant to needed roles
revoke all on function public.register_applicant_with_documents(
  text, date, public.gender_type, text, text, text, text, text,
  boolean, public.source_type, uuid, jsonb
) from public;
...
drop policy if exists "Enable all access for staff" on public.trades;
create policy "Enable all access for staff" on public.trades
  for all using (
...
-- ============================================================
-- DONE.  Your staff account still needs to be mapped:
--
--   INSERT INTO public.staff (id, full_name, role)
--   SELECT u.id, 'Your Name', 'admin'::public.staff_role_type
--   FROM auth.users u
